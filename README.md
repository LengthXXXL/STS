...
![Tests](https://img.shields.io/badge/tests-310_passing-6d3df2?style=flat-square)
...
| 个人空间 | 管理策略、我的积木、模拟账户、回测记录、论坛内容、收藏内容和个人文件 |
...
| 论坛 | 发帖、评论、点赞、收藏，按分类/作者/关联类型筛选，关联公开积木或回测内容，帖子和评论提交后进入审核 |
...
| 公开积木 | `/api/shared-blocks` | 公开列表、详情、收藏、我的收藏、导入、行为记录 |
...
| 论坛 | `/api/forum` | 发帖、评论、点赞收藏、我的收藏、分类/作者/关联类型筛选、我的论坛内容 |
...
| `forum_posts`, `forum_comments`, `forum_post_reactions` | 论坛帖子、评论、审核状态和互动数据 |
...
| 阶段 | 目标 |
| --- | --- |
| 已完成 | 可视化积木搭建、自定义积木参数化、策略保存、模拟账户、回测记录、回测报告导出、A 股/美股规则、行情缓存、个人空间、用户收藏入口和取消收藏、论坛点赞收藏、筛选和热度排序、积木分享、文件管理 |
| 下一步 | 完善收藏批量管理、论坛关联内容的聚合筛选，以及公开积木推荐排序升级 |
